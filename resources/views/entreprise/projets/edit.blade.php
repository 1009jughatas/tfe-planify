...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form method="POST" action="{{ route('entreprise.projets.update', $projet->id) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Description du projet...">{{ old('description', $projet->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" 
                           id="start_date" 
                           name="start_date" 
                           value="{{ old('start_date', $projet->start_date) }}"
                ...
                <div>
                    ...
                    <input type="date" 
                           id="end_date" 
                           name="end_date" 
                           value="{{ old('end_date', $projet->end_date) }}"
                ...
                <div>
                    ...
                    <select id="priority" 
                        ...
                        <option value="low" {{ old('priority', $projet->priority) === 'low' ? 'selected' : '' }}>Faible</option>
                        <option value="medium" {{ old('priority', $projet->priority) === 'medium' ? 'selected' : '' }}>Moyenne</option>
                        <option value="high" {{ old('priority', $projet->priority) === 'high' ? 'selected' : '' }}>Haute</option>
                    </select>
                    @error('priority')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="status" 
                        ...
                        <option value="pending" {{ old('status', $projet->status) === 'pending' ? 'selected' : '' }}>En attente</option>
                        <option value="in-progress" {{ old('status', $projet->status) === 'in-progress' ? 'selected' : '' }}>En cours</option>
                        <option value="completed" {{ old('status', $projet->status) === 'completed' ? 'selected' : '' }}>Terminé</option>
                        <option value="cancelled" {{ old('status', $projet->status) === 'cancelled' ? 'selected' : '' }}>Annulé</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                        ...
                        @forelse($employees as $employee)
                            <div class="flex items-center space-x-3">
                                <input type="checkbox" 
                                       ...
                                       name="participants[]" 
                                       value="{{ $employee->id }}"
                                       {{ in_array($employee->id, old('participants', $projet->participants->pluck('id')->toArray())) ? 'checked' : '' }}
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200 mt-6">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('entreprise.projets.show', $projet->id) }}" class="btn-secondary-modern">
                        <i class="fas fa-eye mr-2"></i>
                        Voir le projet
                    </a>
                    <form method="POST" action="{{ route('entreprise.projets.destroy', $projet->id) }}" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce projet ? Cette action est irréversible.')">
                        @csrf
                        @method('DELETE')
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="text-center">
                    <p class="text-2xl font-bold text-blue-900">{{ $projet->tasks->count() }}</p>
                    <p class="text-sm text-blue-700">Total tâches</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl font-bold text-green-600">{{ $projet->tasks->where('status', 'completed')->count() }}</p>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
            <div class="flex items-start space-x-3">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-yellow-900 mb-1">💡 Conseils</h3>
                    <ul class="text-sm text-yellow-700 space-y-1">
                        <li>• Modifiez le statut pour suivre la progression</li>
                        <li>• Ajustez les participants selon les besoins</li>
                        <li>• Les dates doivent être cohérentes</li>
                        <li>• La priorité aide à organiser le travail</li>
                    </ul>
                </div>
            </div>
        </div>
